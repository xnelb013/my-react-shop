import styled from "./Skeleton.module.css";
// 스켈레톤 모음
export const Skeleton = () => {
  return (
    <div className={styled.skeleton}>
      <div className={styled.imgSkeleton} />
      <div className={styled.desSkeleton}>
        <div className={styled.titleSkeleton} />
        <div className={styled.descriptionSkeleton} />
        <div className={styled.priceSkeleton} />
      </div>
    </div>
  );
};

export const SkeletonCart = () => {
  return (
    <>
      <div className={`${styled.CartContainer} border border-blue-300 shadow rounded-md p-4 max-w-lg w-full mx-auto`}>
        <div className="animate-pulse flex space-x-4">
          <div className="rounded-full bg-slate-200 h-20 w-20"></div>
          <div className="flex-1 space-y-6 py-1">
            <div className="h-4 bg-slate-200 rounded"></div>
            <div className="space-y-3">
              <div className="grid grid-cols-3 gap-4">
                <div className="h-4 bg-slate-200 rounded col-span-2"></div>
                <div className="h-4 bg-slate-200 rounded col-span-1"></div>
              </div>
              <div className="h-4 bg-slate-200 rounded"></div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export const SkeletonDetail = () => {
  return (
    <>
      <div className={styled.DetailWrap}>
        <div className={`${styled.DetailContainer} border border-blue-700 shadow rounded-md `}>
          <div className="animate-pulse flex space-x-4">
            <div className="rounded-full bg-slate-200 h-80 w-80 mr-20"></div>
            <div className="flex-1 space-y-6 py-1 w-60 mt-20">
              <div className="h-10 w-60 bg-slate-200 rounded"></div>
              <div className="space-y-3">
                <div className="grid grid-cols-3 gap-8">
                  <div className="h-10 w-80 bg-slate-200 rounded col-span-2"></div>
                  <div className="h-10 w-40 bg-slate-200 rounded col-span-1"></div>
                </div>
                <div className="h-10 w-60 bg-slate-200 rounded"></div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};
